...
void setScalingMatrix(float sx,float sy,float scalingMatrix[rows][rows])
{
...
void activity()
{
	...
	float tmatrix2[3][3];
	float smatrix [3] [3]; // //step 3 matrix
	setTranslationMatrix(-dx, -dy, tmatrix);
	//setRotationMatrx(teta, rmatrix);
	setScalingMatrix(0.5,0.5,smatrix);
	//setTranslationMatrix(dx, dy, tmatrix);
	...
	multiply_arrays(smatrix, temp, temp2);
